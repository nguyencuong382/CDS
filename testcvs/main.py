...
sign_map = {'traffic_sign': 1, 'pedestrianCrossing': 2}  # only 2 sign classes (background class is 0)
'''
...
path_csv_train = './train__labels.csv'
train_labels = []
with open(path_csv_train, 'r') as f:
	for line in f:
		line = line[:-1]  # strip trailing newline
		train_labels.append(line)

# Create pickle file to represent dataset
image_files = os.listdir(DIR_IMAGE)
	...
	for line in train_labels:
		if re.search(image_file, line):
			...
			if RESIZE_IMAGE:
				...
				image = Image.open(os.path.join(DIR_IMAGE, image_file))
